...
func TestQueryTranslate(t *testing.T) {
	tests := []struct {
		...
		// SQL injection attempt cases
		{
			...
			sqlOut:        "SELECT * FROM `logs_ WHERE 1_1_ DROP TABLE customers_ --` WHERE user_id = @user_id",
		...
		},
		{
			...
			sqlOut:        "SELECT * FROM `users_ UNION SELECT _ FROM passwords WHERE _1___1` WHERE id = @id",
			parametersOut: []bigquery.QueryParameter{{Name: "id", Value: 1}},
		},
		{
			...
			sqlOut:        "DELETE FROM `temp_table__ DELETE FROM important_data_ --` WHERE id = @id",
		...
		},
		{
			...
			sqlOut:        "UPDATE `users_ -- malicious comment` SET status = @status WHERE id = @id",
			parametersOut: []bigquery.QueryParameter{{Name: "status", Value: "active"}, {Name: "id", Value: 1}},
		},
	}
